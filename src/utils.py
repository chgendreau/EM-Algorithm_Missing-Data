...
def MNAR_mask_logistic(X, p, p_params =.3, exclude_inputs=True):
    ...
    depend on masked values. (THE ONE USED IN PRODUCE_NA.PY IS (ii))
...
##################### Our functions #############################
def cluster_mask(X, p_miss, p_obs):
    """
    Generate a missing-data mask for a cluster missing-data mechanism.
    ...
    Returns
    ----------
    A matrix indexing the generated missing values.
    """
    ...
    n_clust = np.random.binomial(d, p_obs)
    # generate the cluster sizes
    clust_sizes = np.random.multinomial(n, [1/d]*d)
    # generate the cluster masks
    clust_masks = [np.random.binomial(1, p_miss, size=clust_sizes[i]) for i in range(d)]
    # generate the cluster indices
    clust_indices = [np.random.choice(n, size=clust_sizes[i], replace=False) for i in range(d)]
    # fill in the mask
    for i in range(n_clust):
        mask[clust_indices[i], i] = clust_masks[i]
